...
const SimilarList = (props)=>{
    ...
    useEffect(()=>{
        const get_list = async ()=>{
            try{
                ...
                const list = await fetch(`${env.REACT_APP_STORAGE_URL}/get_series`,{
                    method: "POST",
                    headers: {"Content-Type" : "application/json"},
                    body : JSON.stringify({genres: result.genre_list}),
                }).then(data=>data.json());
                set_series(list.result);
            }
    ...
    return(
        <div className="list">
            <label className="center_box">Recommended</label>
            {series.map((v,i)=>v.title !== title && v.season !== season ? <Card {...v} key={`similar ${i}`}/>:null)}
        </div>
    )
}
